...
Woodcutting needs a frontend cleanup pass after the backend parity work: target rows should render `display_name`/`output_item` from `TREE_DATA` instead of raw stable IDs, show the best usable hatchet and lock reason, label Ivy as XP-only/no log, and surface `Open bird nests` with clear no-XP batch wording. The backend already exposes the data; avoid re-deriving source rules in Qt.
