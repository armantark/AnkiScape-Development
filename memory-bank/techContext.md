...
- **Ivy is intentionally iconless.** Ivy has no inventory/log item, and `File:Ivy.png` (OSRS) is a full castle-wall *screenshot*, not an icon — unusable in a 28px row. The row already reads "— XP only", so it stays blank rather than shipping a scene grab. Egg bird-nests and bird's eggs are likewise iconless (wiki titles don't resolve; vanishingly rare drops). Nest *ring* contents reuse the existing crafted ring sprites.
