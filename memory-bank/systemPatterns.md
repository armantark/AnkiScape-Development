...
- The hybrid toolbelt pattern now covers Mining too. Toolbelt remains tools-only (`bronze_hatchet`, `bronze_pickaxe`, etc.). Passive/equipable bonuses should live outside toolbelt; the temporary backend-only `owned_equipment` collection handles Mining bonus items until a real equipment UI exists.
